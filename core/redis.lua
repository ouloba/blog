...
function response.read(client)
	...
	--PrintFastLog('prefix->', prefix, ' data->', data, ' payload->', payload)
    -- status reply
    if prefix == '+' then
        if data == 'OK' then
            return true
...
local function connect_tcp(socket, parameters)
	...
	--PrintFastLog('connect_tcp connecting host:', host, 'port:',port)
    local ok, err = socket:connect(host, port)
    if not ok then
        redis.error('could not connect to '..host..':'..port..' ['..err..']')
    end
	PrintFastLog('connect_tcp ok')
...
local function create_connection(parameters)
    ...
    if parameters.scheme == 'unix' then
        ...
        perform_connection, socket = connect_tcp, require('coresocket').tcp
    end

	PrintFastLog('socket:', socket and 'ok' or 'nil')
...
function redis.error(message, level)
	PrintFastLog('message:', message, 'level:', level)
...
function redis.connect(...)
    ...
    elseif #args > 1 then
		...
		PrintFastLog('host:', host, 'port:',port, 'timeout:', timeout)
